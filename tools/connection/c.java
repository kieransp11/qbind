...
public class c{
  ...
  */
   ...
   *    [isLittleEndian, msgType, isCompressed, original = 0]
   * First 4 bytes are [original, original, 1, original]. TODO: check what these bytes are
   *    [on write uncompressed we put length, and read we skip]
   * Next 4 bytes: 0 0 0 0
   * Then 4 bytes for original length. 
   * 
   * Then blocks start. 
   */
  private void compress(){
    byte i=0;
    boolean g;
    final int wBuffOrigLength=wBuffPos; // constant wBuffPosOrig
    int f=0;    // f is at bitmask where g is false. Ends up being block header.
    int h0=0;
    int h=0;
    byte[] wBuffOrig=wBuff; // backup old buffer. Never modified
    wBuff=new byte[y.length/2]; // new buffer half the length
    int c=12; // index to go back to and put block header
    int wBuffNewIdx=c; // d
    int wBuffLengthNew=wBuff.length; // never modified. Always half of original buffer length
    int p=0;
    int endOfWindowIdx; // some kind of lookahead window. Maximum 255. q
    int wBuffOrigRunStartIdx; // r
    int s0=0; // last iterations wBuffOrigIdx
    int wBuffOrigIdx=8; // s
    //int t=wBuffPos; // never modified Unused
    // a is a list of indices of the last time the offset occurred.
    int a[]=new int[256];
    System.arraycopy(wBuffOrig,0,wBuff,0,4); // copy first 4 bytes from old buffer to new
    wBuff[2]=1;
    wBuffPos=8;

    // write original size to wBuff (increments wBuffPos)
    w(wBuffOrigLength);

    for(;wBuffOrigIdx<wBuffOrigLength;i*=2){                                    // i1
      if(0==i){       // executes every 8 iterations as overflow eventually gets back to zero.
        // if you're within 17 bytes then return original buffer as wont be <1/2 original size.
        // A block is max 17 bytes. 2 bytes per iteration and a header.
        if(wBuffNewIdx>wBuffLengthNew-17){
          wBuffPos=wBuffOrigLength;
          wBuff=wBuffOrig;
          return;
        }
        i=1;                                                                    // i2
        // f is some mask which tells you if its a one get the next two bytes, else next one byte
        wBuff[c]=(byte)f; // how come this can be a byte? => get 8 least significant bits
        c=wBuffNewIdx++;                                                        //    c1 d1                     only time c is modified. d modified once.
        f=0;                                                                    //          f1                  f modified once
      }

      g=(wBuffOrigIdx>wBuffOrigLength-3)||                                      //             g1               g modified once
        (0==(                                                                   //                                                  => index of a never assigned. Where two adjacent indices have an xor of h.
            p=a[                                                                //                p1            p modified first. 
                h=0xFF&(wBuffOrig[wBuffOrigIdx]^wBuffOrig[wBuffOrigIdx+1])      //                   h1         only time h is modified: h = an index into a between 0 and 255.
                ]
            )
        )||
        (0!=(wBuffOrig[wBuffOrigIdx]^wBuffOrig[p]));    // (in last 3 indices) or (two adjacent indices of wBufferOrig have not had that xor difference) or (the start of the runs have different values - xor != 0 => is a != b)

      // set in a that an offset has occurred more recently
      // if same offset keeps occurring between successive elements this boils down to just writing each number
      if(0<s0){ // if g last iteration
        a[h0]=s0;                                       // a modified once
        s0=0;                                           // s0 is modified once
      }

      if(g){
        h0=h;                                                           // only time h0 is modified - a array index.                                    => h is index the last time this offset occurred
        s0=wBuffOrigIdx;                                                // s0 modified twice        - a array value                                     => s0 is index when the offset occurred this time.
        wBuff[wBuffNewIdx++]=wBuffOrig[wBuffOrigIdx++];                 // d modified twice. s modified once - take byte directly from original buffer  => write the value starting this new run to the new buffer. 
      }else{
        // if you're here you know
        //  1) The offset between two adjacent indices has occurred before
        //  2) The start value when the offset occurred last time is the same.
        a[h]=wBuffOrigIdx;                                              // a modified twice => set where the offset last happened
        f|=i;                                                           // f modified twice                             => f is basically a bit mask of when run length encoding is used. 
        p+=2;                                                           // p modified twice.                            => index after last time this offset occurred
        wBuffOrigRunStartIdx=wBuffOrigIdx+=2;                           // only time r is modified. s modified twice    => index after this time this offset occurred
        endOfWindowIdx=Math.min(wBuffOrigIdx+255,wBuffOrigLength);      // only time q is modified.
        while(wBuffOrig[p]==wBuffOrig[wBuffOrigIdx]&&++wBuffOrigIdx<endOfWindowIdx)  // s modified third time.
          ++p;                                                          // p modified third time
        wBuff[wBuffNewIdx++]=(byte)h;                                   // d modified third.  => store the offset (h)
        wBuff[wBuffNewIdx++]=(byte)(wBuffOrigIdx-wBuffOrigRunStartIdx); // d modified fourth. => store the length of the run.
      }

    }
    wBuff[c]=(byte)f;
    wBuffPos=4;
    w(wBuffNewIdx); // write d to buffer. Write final length.
    wBuffPos=wBuffNewIdx;
    wBuff=Arrays.copyOf(wBuff,wBuffPos); // make a copy and remove unused data. i.e. realloc
  }
  private void uncompress(){
    int runLength=0; // n
    int r=0;
    int f=0;
    int dstIdx=8; // s => after the header
    int p=dstIdx;
    short i=0;
    byte[] dst=new byte[ri()]; // length of original message in bytes
    int rBuffPosLoc=rBuffPos; // d => will be 12 now after ri(). i.e. ready to read data.
    int[] aa=new int[256];

    while(dstIdx<dst.length){
      if(i==0){
        f=0xff&(int)rBuff[rBuffPosLoc++]; // get compression block header
        i=1; // set mask
      }

      if((f&i)!=0){
        r=aa[0xff&(int)rBuff[rBuffPosLoc++]]; // first byte of block (h)
        dst[dstIdx++]=dst[r++];
        dst[dstIdx++]=dst[r++];
        runLength=0xff&(int)rBuff[rBuffPosLoc++]; // second byte of block 
        for(int m=0;m<runLength;m++)
          dst[dstIdx+m]=dst[r+m];
      }else
        dst[dstIdx++]=rBuff[rBuffPosLoc++]; // initial value of buffer.
    ...
    // reset read buffer
    rBuff=dst;
    rBuffPos=8; // same as initial dstIdx
  }
  /**
  ...
  Object r() throws UnsupportedEncodingException{
    int i=0;    // index for deserialising arrays
    int n;      // length of array
    int t=rBuff[rBuffPos++]; // type as integer.
    // integer
    if(t<0)
      switch(t){
        case -1:
    ...
    if(t>99){
      ...
      // unary primitive/operator/iterator/projection
      if(t<104)
        return rBuff[rBuffPos++]==0&&t==101?null:"func";
      // f', f/, f\, f':, f/:, f\:, dynamic load
      if(t>105)
        r();
      // 104 or 105 (projection or composition)
      else
        for(n=ri();i<n;i++)
          r();
      return "func";
