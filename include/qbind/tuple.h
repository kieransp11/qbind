...
template<typename T>
typename std::tuple_element<type_to_idx<T, Types...>(), Tuple<Types...>>::type get()
{
...
template<typename T>
const typename std::tuple_element<type_to_idx<T, Types...>(), Tuple<Types...>>::type get() const
{
    return T{K::make_non_owning(
        get_idx(
            type_to_idx<T, Types...>()
...
struct isTuple<Tuple<Args...>> : public std::true_type
{ };
